...
//5felder,15felder,25felder,35felder,$10000,$50000,$100000,$1000000,10runden,50runden,100runden,1000runden,shopper,tutorial
...
var fluid=new Array("bugKiller","Wasser"),gebäude=new Array("coffeeshop");
...
document.getElementById("tip").value=tips[tipnum]
	...
	var id=setTimeout("willkommen()",1);
}
function tipalert() {
...
confirm ("Willkommen auf deiner neuen Farm!")
confirm ("Hier im Tutorial wirst du zuerst die Grundlagen des Spiels kennenlernen.")
confirm ("Wie im echten Leben beginnt es hier mit dem Grubbern. Klicke hierzu auf den 'Grubbern'-Button, bis er grün leuchtet!")
tutstep=1
	document.getElementById("loading_screen").style.display="none"
...
function setaction(setzen) {
	...
	if(action==1&&tutstep==1) {confirm("Gut gemacht! Nun klicke auf das erste Feld in der Reihe, um es zu grubbern.");tutstep=2}
	if(action==3&&tutstep==5) {confirm("Gut gemacht! Nun klicke auf das erste Feld in der Reihe, um es zu säen.");tutstep=6}
	if(action==3) {
...
function update() {
	...
	if (rundenzeit==-1) {
		rundenzeit=100
		checkfields()
		if((felderinbesitz[1][1]==5||felderinbesitz[1][1]==5)&&tutstep==10){confirm("Hey! Eines deiner Felder ist erntereif! Klicke auf den 'Ernten'-Button, um dein Feld zu ernten, bevor es verdorrt.");tutstep=11}
	}
	curcheckachs()
...
function changeobj() {
	...
	if(tutstep==8){confirm("Gut! Klicke jetzt das zweite Feld in der Reihe, um es zu besäen.");tutstep=9}
}
function minimize() {
...
function sellwheat() {
	...
	if (tutstep==14){confirm("Das ist der Marktplatz. Du hast von jedem 11, also gib an jeder Nummer eine 11 ein und drücke an jedem einmal den 'Verkaufen'-Button!");tutstep=15}
...
confirm("Verkaufe ruhig alles was du hast, du wirst es hier nicht mehr brauchen!")
	if (objekt=="wheat") {
	...
	if(tutstep==15&&wheat==0&&maize==0){var ttz=confirm("Gut gemacht! Das einzige, was man dir jetzt noch erklären könnte, wäre die Insektenplage, aber das wird dir genauer erklärt, wenn es soweit ist. Willst du dein Spiel hier fortsetzen?")
	tutstep=16
	alert("Erfolg erzielt","Du hast den Erfolg 'Schlaumeier' Freigeschaltet! Zur Belohnung erhälst du $100.");money+=100;document.getElementById("tuto_ach").style.opacity="1";erfolge[14]=true;document.getElementById("erfolg").play()
	if(ttz!=true){window.open("index.html")}
	}
}
...
if(tutstep==7){confirm("Fertig! Besäe nun das andere Feld mit Mais, indem du auf 'Weizen' in dem Säen-Knopf klickst und Mais wählst.");tutstep=8}
...
if(tutstep==9){confirm("Fertig! Jetzt musst du warten, bis eins deiner Felder erntereif ist.");tutstep=10}
	traktor[0][1]-=1
...
function grubbere(feldid) {
	if(tutstep==3){confirm("Das erste Feld ist fertig! Jetzt grubbere auch das zweite, um mehr Platz zu schaffen.");tutstep=4}
	else if(tutstep==4){confirm("Super! Du hast zwei Felder gegrubbert.Die darfst du jetzt besäen!Klicke dazu auf den 'Feld besäen'-Knopf bis er grün leuchtet.");tutstep=5}
	grubber[0][1]-=1;
	traktor[0][1]-=1;
...
var maisernte=false,weizenernte=false;
function ernte_maize(feldid) {
if(maisernte==false){maisernte=true}
	if(tutstep==12&&maisernte==true&&weizenernte==true){
	confirm("Gut! Du könntest diese Felder auch wieder grubben, aber für das Tutorial lassen wir es gut sein.")
	confirm("Du hast nun etwas Weizen und Mais. Lass ihn uns am Markt verkaufen! Klicke dazu auf das Kleine Haus mit den Münzen am unteren Spielfeldrand.")
	tutstep=13
}
	maehdresch[0][1]-=1
	document.getElementById("maize").value=maize
...
if(weizenernte==false){weizenernte=true;confirm("OK!Hast du das andere auch schon?")}
if(tutstep==12&&maisernte==true&&weizenernte==true){
	confirm("Gut! Du könntest diese Felder auch wieder grubben, aber für das Tutorial lassen wir es gut sein.")
	confirm("Du hast nun etwas Weizen und Mais. Lass ihn uns am Markt verkaufen! Klicke dazu auf das Kleine Haus mit den Münzen am unteren Spielfeldrand.")
	tutstep=13
...
function feldaction(feldid) {
	...
	if (felderinbesitz[feldid][4]!=true) {
		...
		if (action==1) {
		if(tutstep==2){confirm("Jetzt musst du ein bisschen warten!");tutstep=3}
		grubber[0][1]*=1
		grubber[0][0]*=1
		grubber[0][1]+=1
		traktor[0][0]*=1
	...
	if(tutstep==11){confirm("Gut! Klicke nun auf das reife Feld, um es zu ernten.");tutstep=12}
		maehdresch[0][0]*=1
		maehdresch[0][1]*=1
		maehdresch[0][1]+=1
	...
	if(tutstep==6){confirm("Jetzt musst du wieder ein bisschen warten!");tutstep=7}
...
function nachtmachen() {
	...
	var ir=setTimeout("document.getElementById('body').style.opacity='0.2';document.getElementById('moon').style.display='none'",3000)
}
function showachs() {
